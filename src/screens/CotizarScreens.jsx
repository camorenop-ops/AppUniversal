...
import {
  PRODUCT_TITLES, CATEGORIA, BASE_PERSONA, EDAD_FACTOR, AUTO_FACTOR, PROPIEDAD_FACTOR,
...
function PersonaDatosStep({ title }) {
  ...
  return (
    <>
      ...
      <ChipGroup options={["18-30", "31-45", "46-60", "60+"]} current={cot.edad} field="edad" />
      ...
      <StepNav onBack={prevCot} onForward={nextCot} forwardLabel="Continuar" disabled={!(cot.sexo && cot.edad && cot.parentesco)} />
    </>
  );
}
...
function OfertaComparadaStep({ title }) {
  ...
  if (cat === "persona") {
    const factor = EDAD_FACTOR[cot.edad];
...
export function SaludFamiliaScreen() {
  const { openStub, openAgregarCobertura } = useApp();
  return (
    <>
      <BackHeader title="Seguro de Salud" />
      ...
      <button onClick={() => openStub("Cambio de plan")} style={{ width: "100%", marginBottom: 8 }}>Cambiar de plan</button>
