...
import { TITULAR_NOMBRE, AFILIADOS_SALUD_INFO, LIMITE_POR_CASO_PLAN, COBERTURA_MEDICAMENTOS_PLAN } from "../data/data";
...
export function AfiliadoDetalleScreen() {
  ...
  const medicamentosTotal = COBERTURA_MEDICAMENTOS_PLAN[plan];
  ...
  return (
    <>
      ...
      <div style={{ fontSize: 13, color: "var(--muted)", marginBottom: 14 }}>{info.parentesco} · {plan}</div>
      <SectionLabel>Coberturas y saldos disponibles</SectionLabel>
      <div onClick={() => openAfiliadoCobertura(nombre, "limite")} className="card" style={{ marginBottom: 10, cursor: "pointer" }}>
      ...
      <div onClick={() => openAfiliadoCobertura(nombre, "medicamentos")} className="card" style={{ cursor: "pointer" }}>
...
export function AfiliadoCoberturaHistorialScreen() {
  ...
  const listaReembolsos = esTitular ? reembolsos : info.reembolsos;
  const tituloCobertura = tipo === "limite" ? "Límite por caso" : "Cobertura de medicamentos";

  return (
    <>
      <BackHeader title={`${tituloCobertura} · ${nombre}`} />
